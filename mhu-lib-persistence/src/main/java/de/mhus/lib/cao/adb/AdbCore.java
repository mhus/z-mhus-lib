...
public class AdbCore extends CaoCore {
	...
	public AdbNodeData[] getChildren(UUID parent) throws MException {
		return manager.getByQualification(Db.query(AdbNodeData.class, type).eq("collection", collection).eq("parent", parent)).toArrayAndClose(new AdbNodeData[0]);
	}

	public AdbNodeData getChild(UUID parent, String name) throws MException {
		return manager.getObjectByQualification(Db.query(AdbNodeData.class, type).eq("collection", collection).eq("parent", parent).eq("name", name) );
	}
	
	public AdbNodeData[] getChildren(UUID parent, String name) throws MException {
		return manager.getByQualification(Db.query(AdbNodeData.class, type).eq("collection", collection).eq("parent", parent).eq("name", name) ).toArrayAndClose(new AdbNodeData[0]);
	}
