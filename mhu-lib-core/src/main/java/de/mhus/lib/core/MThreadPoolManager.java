...
public class MThreadPoolManager extends MObject implements IBase {
	...
	public static CfgLong CFG_SLEEP_TIME = new CfgLong(MThreadPoolManager.class, "sleepTime", 1000 * 60 * 10);
	public static CfgLong CFG_PENDING_TIME = new CfgLong(MThreadPoolManager.class, "pendingTime", 1000 * 60);
	...
	ThreadContainer start(MThreadPool _task, String _name) {
		...
		synchronized (pool) {
			if (housekeeper == null) {
				...
				MApi.lookup(MHousekeeper.class).register(housekeeper, CFG_SLEEP_TIME.value());
			}
			// search free thread
