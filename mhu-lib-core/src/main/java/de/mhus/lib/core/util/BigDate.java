...
/**
 * Convert Gregorian YYYY MM DD back and forth to ordinal days since 1970 Jan 1,
 * Thursday (sometimes called Julian or datestamp form). BigDate objects are not
 * designed for <b>storing</b> dates in a database, just for conversion. Long
 * term storage should store the ordinal either as an int, or possibly as a
 * short. The BigDate constructor stores the date both in ordinal and Gregorian
 * forms internally. If you store one, it creates the other. <p/> The standard
 * Sun Date won't handle dates prior to 1970 among other problems. BigDate
 * handles dates 999,999 BC Jan 1 to 999,999 AD Dec 31, 0 = 1970 Jan 1. <p/> Are
 * the following quirks of the calendar considered in this code?<br>
 * <dl compact> <dt>yes</dt> <dd>1900 is not a leap year (mod 100).</dd> <dt>yes
 * </dt> <dd>2000 is a leap year (mod 400).</dd> <dt>yes</dt> <dd>The 10 missing
 * days in 1582 October. (Pope Gregory's correction) 1582 Oct 5 to 1582 Oct 14
 * never happened.</dd> <dt>partly</dt> <dd>Britain and its territories
 * (including the USA and Canada) adopted the Gregorian correction in 1752. By
 * then, 11 days had to be dropped. 1752 September 3 to 1752 September 13 never
 * happened. However, you can modify constants in BigDate to use the British
 * calendar. Such a change only affects dates prior to 1753 since BigDate
 * calendar is based on Jan 1, 1970. toOrdinal with the Gregorian and British
 * scheme will give the same number for recent dates. You must recompile with
 * the isBritish boolean changed to true.</dd> <dt>yes</dt> <dd>missing year 0
 * between 1 BC and 1 AD.</dd> <dt>no</dt> <dd>in Roman times leap years
 * occurred at irregular intervals, Considered inauspicious, they were avoided
 * during war. Instead we presume leap years every 4 years even back to
 * 999,999BC.</dd> <dt>no</dt> <dd>leap seconds.</dd></dl> <p/> Normally all you
 * need is one BigDate object that you use for all interconversions with
 * set(ordinal), set(yyy,mm,dd) and getOrdinal(), getYYYY(), getMM(), getDD().
 * <p/> java.util.Date has some odd habits, using 101 to represent the year
 * 2001, and 11 to represent December. BigDate is more conventional. You use
 * 2001 to represent the year 2001 and 12 to represent December. <p/> BigDate
 * implements proleptic Gregorian and Julian calendars. That is, dates are
 * computed by extrapolating the current rules indefinitely far backward and
 * forward in time. As a result, BigDate may be used for all years to generate
 * meaningful and consistent results. However, dates obtained using BigDate are
 * historically accurate only from March 1, 4 AD onward, when modern Julian
 * calendar rules were adopted. Before this date, leap year rules were applied
 * irregularly, and before 45 BC the Julian calendar did not even exist. Prior
 * to the institution of the Gregorian calendar, New Year's Day was March 25. To
...
public class BigDate implements Cloneable, Serializable, Comparable<Object> {
	 ...
	 * converted into the equivalent valid one.<br/> e.g. 1954 September 31 ->
	 * 1954 October 1. <br/> 1954 October -1 -> 1954 September 29.<br/> 1954 13
	 * 01 -> 1955 01 01.
	 */
	...
	/**
	 * PLEASE CONFIGURE isBritish BEFORE COMPILING. Mysterious missing days in
	 * the calendar. Pope Gregory: 1582 Oct 4 Thursday, was followed immediately
	 * by 1582 Oct 15 Friday dropping 10 days. British: 1752 Sep 2 Wednesday was
	 * followed immediately by 1752 Sep 14 Thursday dropping 12 days. Constant:
	 * true if you want the British calender, false if Pope Gregory's. You must
	 * recompile for it to have effect. For Britain, the USA and Canada it
	 * should be true.
	 */
	public static final boolean isBritish = true;

	/**
	 * Constant: adjustment to make ordinal 0 come out on 1970 Jan 01 for AD
	 * date calculations. This number was computed by making an estimate, seeing
	 * what value toOrdinal gave for 1970/01/01 and then adjusting this constant
	 * so that 1970/01/01 would come out Ordinal 0.
	 */
	private static final int AD_epochAdjustment;

	/**
	 * Constant: adjustment to make ordinal 0 come out to 1970 Jan 01 for BC
	 * date calculations. Account for missing year 0.
	 */
	...
	/**
	 * Constant: Ordinal for 1582 Dec 31, the last day of first year of the
	 * Gregorian calendar.
	 */
	private static final int GC_firstDec_31;

	/**
	 * Constant: month of the first date of the Gregorian Calendar. Pope
	 * Gregory: 1582 Oct 15, British: 1752 Sep 14
	 */
	private static final int GC_firstMM;

	/**
	 * Constant: Ordinal for 1582 Oct 15, the first day of the Gregorian
	 * calendar.
	 */
	private static final int GC_firstOrdinal;

	/**
	 * Constant: year of the first date (1752 Oct 15) of the Gregorian Calendar
	 * = 1582. Just after the missing daysp. Different parts of the world made
	 * the transition at different times. Pope Gregory: 1582 Oct 4, British:
	 * 1752 Sep 2
	 */
	...
	/**
	 * Constant: ordinal of 1600 Jan 01, the first year when the mod 100 leap
	 * year rule first had any effect.
	 */
	private static final int Jan_01_Leap100RuleYear;

	/**
	 * Constant: ordinal of 1600 Jan 01, the year when the mod 400 leap year
	 * rule first had any effect.
	 */
	private static final int Jan_01_Leap400RuleYear;

	/**
	 * Constant: year that the mod 100 rule first had any effect. For The
	 * Gregorian Calendar, 1600. For the British Calendar, 1800. Round up to
	 * next 100 years after the missing day anomaly.
	 */
	private static final int Leap100RuleYYYY;

	/**
	 * Constant: year that the mod 400 rule first had any effect. For The
	 * Gregorian Calendar, 1600. For the British Calendar, 2000. Round up to
	 * next 400 years after the missing day anomaly.
	 */
	...
	/**
	 * Constant: day of the last day of the month of the old Julian calendar.
	 * Pope Gregory: 1582 Oct 4, British: 1752 Sep 2
	 */
	private static final int OJC_lastDD;

	/**
	 * Constant: month of the last date of the old Julian Calendar Pope Gregory:
	 * 1582 Oct 4, British: 1752 Sep 2
	 */
	private static final int OJC_lastMM;

	/**
	 * Constant: year of the last date (1582 Oct 4) of the old Julian calendar,
	 * just prior to the missing 10 days, = 1852. Different parts of the world
	 * made the transition at different times. Usually 1582 Oct 4. For British
	...
	/**
	 * Constant array: how many days in the year prior to the first of the given
	 * month in a leap year. Indexed by Jan=0.
	 */
	...
	/**
	 * Constant array: what month does the indexing day number fall in in a leap
	 * year? Indexed by ddd Jan 1 = 0.
	 */
	private static int[] leap_dddToMMTable;

	/**
	 * Constant: how many days were lost during the Gregorian correction, 10 for
	 * the Gregorian calendar, 11 for the British.
	 */
	 ...
	 */
	private static final String[] shortDayName = { "sun", "mon", "tue", "wed",
			"thu", "fri", "sat" };
	...
	/**
	 * Constant array: how many days in the year prior to the first of the given
	 * month in a non-leap year. Indexed by Jan=0.
	 */
	...
	/**
	 * Constant array: what month does the indexing day number fall in in a
	 * non-leap year? Indexed by ddd Jan 1 = 0.
	 */
	private static int[] usual_dddToMMTable;

	/**
	 * Day, 1 to 31. If size of BigDate objects were a consideration, you could
	 * make this a byte.
	 */
	protected transient int dd = 0;

	/**
	 * Month, 1 to 12. If size of BigDate objects were a consideration, you
	 * could make this a byte.
	 */
	protected transient int mm = 0;

	/**
	 * Ordinal days since Jan 01, 1970. -365968798 to 364522971. i.e. 999,999 BC
	 * to 999,999 AD.
	 */
	...
	static {
		...
		GC_firstOrdinal = BigDate.toOrdinal(GC_firstYYYY, GC_firstMM,
				GC_firstDD);
	 ...
	 * right now. It works even if Java's default Timezone is not configured
	 * correctly, but it requires your system clock accurately set to UTC time.
	 * Experiment setting your system date/time to various values and making
	...
	public static int dayOfWeek(int ordinal) {
		...
		return (ordinal == NULL_ORDINAL) ? 0 : ((ordinal
				+ SundayIsZeroAdjustment - MIN_ORDINAL) % 7);
	}
	 ...
	 * rules? By 1582, this excess of leap years had built up noticeably. At the
	 * suggestion of astronomers Luigi Lilio and Chistopher Clavius, Pope
	 * Gregory XIII dropped 10 days from the calendar. Thursday 1582 October 4
	 * Julian was followed immediately by Friday 1582 October 15 Gregorian. He
	 * decreed that every 100 years, a leap year should be dropped except that
	 * every 400 years the leap year should be restored. Only Italy, Poland,
	 * Portugual and Spain went along with the new calendar immediately. One by
	 * one other countries adopted it in different years. Britain and its
	 * territories (including the USA and Canada) adopted it in 1752. By then,
	 * 11 days had to be dropped. 1752 September 2 was followed immediately by
	 * 1752 September 14. The Gregorian calendar is the most widely used scheme.
	 * This is the scheme endorsed by the US Naval observatory. It corrects the
	...
	public static boolean isValid(int yyyy, int mm, int dd) {
		...
		if ((yyyy < MIN_YEAR) || (yyyy > MAX_YEAR) || (mm < 1) || (mm > 12)
				|| (dd < 1) || (dd > 31)) {
			return false;
		}
		...
		if (yyyy == OJC_lastYYYY && mm == OJC_lastMM && OJC_lastDD < dd
				&& dd < GC_firstDD) {
			return false;
		}
	...
	public static int isoDayOfWeek(int ordinal) {
		...
		// See modulus in the Java glossary for more details.
		return (ordinal == NULL_ORDINAL) ? 0 : ((ordinal
				+ MondayIsZeroAdjustment - MIN_ORDINAL) % 7) + 1;
	}
	...
	/**
	 * Returns a BigDate object initialised to today's local date. It depends on
	 * Java's default Timezone being configured, and your system clock
	 * accurately set to UTC time. Experiment setting your system date/time to
	 * various values and making sure you are getting the expected results. Note
	 * the date in the created object does not keep updating every time you
	 ...
	 *            6=Saturday WARNING: not compatible with 1=Calendar.SUNDAY.
	 * @param yyyy
	 *            year of interest.
	 * @param mm
	 *            month 1 to 12 (not 0 to 11 as in Sun's Date)
	...
	public static int nthXXXDay(int which, int dayOfWeek, int yyyy, int mm) {
		...
		int dayOfMonthOfNthDesiredDay = dayOfMonthOfFirstDesiredDay
				+ (which - 1) * 7;
	...
	static public int toOrdinal(int yyyy, int mm, int dd) {
		...
		return jan01OfYear(yyyy) + daysInYearPriorToMonth(mm, isLeap(yyyy))
				- missingDayAdjust + dd - 1;
	} // end toOrdinal

	/**
	 * Returns a BigDate object initialised to the date right now in the given
	 * timezone. It depends on your system clock accurately set to UTC time.
	 * Experiment setting your system date/time to various values and making
	 * sure you are getting the expected results. Note the date in the created
	 * object does not keep updating every time you reference it with methods
	 * like getOrdinal or getDD. You always get the date the object was created.
	 ...
	 * @param timeZone
	 *            Which timeZone do you want to know the date for that UTC time.
	...
	public final java.util.Date getDate(TimeZone timeZone) {
		return ordinal == NULL_ORDINAL ? null : new java.util.Date(
	 ...
	 * rules of ISO standard IS-8601 section 5.5. A week that lies partly in one
	 * year and partly in another is assigned a number in the year in which most
	 * of its days lie. This means that week 1 of any year is the week that
	 * contains 4 January, or equivalently week 1 of any year is the week that
	 * contains the first Thursday in January. Most years have 52 weeks, but
	 * years that start on a Thursday and leap years that start on a Wednesday
	 * have 53 weeks. Jan 1 may well be in week 53 of the previous year! Only
	...
	/**
	 * Get milliseconds since 1970 Jan 01 00:00 GMT for this BigDate. Does not
	 * account for leap seconds primarily because we do not know them in
	 ...
	 * follow the rules of ISO standard IS-8601 section 5.5. Week 1 is the first
	...
	/**
	 * Clean up an invalid date, leaving the results internally.<br/> e.g. 1954
	 * September 31 -> 1954 October 1. <br/> 1954 October -1 -> 1954 September
	 * 29.<br/> 1954 13 01 -> 1955 01 01. This lets you do year, month or day
	...
	protected final void normalise() {
		...
		throw new IllegalArgumentException("date cannot be normalised: " + yyyy
				+ "/" + mm + "/" + dd);
	} // end normalise

	/**
	 * read back a serialized BigDate object and reconstruct the missing
	 * transient fields. readObject leaves results in this. It does not create a
	 * new object.
	 * 
	 * @param s
	 *            stream to read from.
	...
	public final void set(int yyyy, int mm, int dd, int how) {
		...
		case CHECK:
			if (!isValid(yyyy, mm, dd)) {
				throw new IllegalArgumentException("invalid date: " + yyyy
						+ "/" + mm + "/" + dd);
			}
			break;
	 ...
	 * @param timeZone
	 *            Timezone you want to know the date in at that time.
	...
	public void setDateAtTime(long utcTimestamp, TimeZone timeZone) {
		 ...
		 * to GMT to get local time.
		 */
		int offsetInMillis = timeZone.getOffset(getOrdinal() >= Jan_01_0001 ? 1
				: 0, getYYYY(), getMM() - 1, getDD(), getCalendarDayOfWeek(),
	...
	public String toDowMMDDYY() {
		...
		return shortDayName[getDayOfWeek()] + " " + MString.toLZ(mm, 2) + "/"
				+ MString.toLZ(dd, 2) + "/" + MString.toLZ(yyyy % 100, 2);
	} // end toDowMMDDYY
	...
	protected final void toGregorian() {
		...
		if (ordinal > MAX_ORDINAL) {
			...
			yyyy = Leap400RuleYYYY
					+ flooredMulDiv(ordinal - Jan_01_Leap400RuleYear, 10000,
	...
	/**
	 * Serialize and write the BigDate object. Only the ordinal will be written.
	 * Other fields are transient.
	 * 
	 * @param s
	 *            stream to write to
	 * 
