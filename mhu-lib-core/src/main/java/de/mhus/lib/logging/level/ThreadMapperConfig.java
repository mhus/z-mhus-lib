...
public class ThreadMapperConfig implements LevelMapper {
    ...
    public void doConfigure(String source, String config) {
        ...
        if (config.equals(MLogUtil.MAP_LABEL)) {
            config = "";
        } else if (config.startsWith("MAP,")) // remove 'MAP,'
        ...
        if (source == null) source = MLogUtil.TRAIL_SOURCE_OTHER;
        source = source.toUpperCase();

        if (source.equals(MLogUtil.TRAIL_SOURCE_REST)) config = MLogUtil.DEFAULT_TRAIL_REST.value();
        else if (config.length() == 0) {
            if (source.equals(MLogUtil.TRAIL_SOURCE_SHELL))
                config = MLogUtil.DEFAULT_TRAIL_SHELL.value();
            else config = MLogUtil.DEFAULT_TRAIL_CONFIG.value();
        }

        String[] parts = config.toUpperCase().split(",");

        if (parts.length > 0) trace = toLevel(parts[0]);
        if (parts.length > 1) debug = toLevel(parts[1]);
        if (parts.length > 2) info = toLevel(parts[2]);
        if (parts.length > 3) warn = toLevel(parts[3]);
        if (parts.length > 4) error = toLevel(parts[4]);
        if (parts.length > 5) fatal = toLevel(parts[5]);
        if (parts.length > 6) local = parts[6].equals("L");
        if (parts.length > 7) setTimeout(MCast.tolong(parts[7], 0));
        if (parts.length > 8) id = parts[8];

        if (id == null) id = source.substring(0, 1) + MLogUtil.createTrailIdent();
